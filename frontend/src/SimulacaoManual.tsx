...
import { simularManual, SimulacaoManualPayload, SimulacaoManualResponse } from "../simulacaoManualApi";
...
export const SimulacaoManual: React.FC = () => {
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) {
    ...
    // strings opcionais: vazio => null
    const nullableFields: (keyof SimulacaoManualPayload)[] = ["ncm", "produto", "movimento", "cfop"];
  ...
  const resumoPorTributo = useMemo(() => {
    if (!result) return [];
    return [
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold text-slate-800">Calculadora da Reforma — Simulação Manual</h1>
        <p className="text-sm text-slate-500 mt-1">
          Preencha as bases e alíquotas atuais. CBS/IBS são obtidos pelos parâmetros (JSON) com fallback.
        </p>
      </header>

      <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <h2 className="font-semibold text-slate-700">Parâmetros da simulação manual</h2>

        <div className="grid gap-4 md:grid-cols-3">
          <div>
            <label className="block text-sm text-slate-600 mb-1">Ano do cronograma (2026–2033)</label>
            <input type="number" name="ano" min={2026} max={2033} value={form.ano} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">Valor do produto (R$)</label>
            <input type="number" name="valor_produto" value={form.valor_produto} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">BC ICMS (R$)</label>
            <input type="number" name="bc_icms" value={form.bc_icms} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">BC PIS (R$)</label>
            <input type="number" name="bc_pis" value={form.bc_pis} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">BC COFINS (R$)</label>
            <input type="number" name="bc_cofins" value={form.bc_cofins} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">Alíquota ICMS atual (decimal)</label>
            <input type="number" step="0.0001" name="aliq_icms_atual" value={form.aliq_icms_atual} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
            <div className="text-xs text-slate-400 mt-1">Ex.: 0,04 = 4%</div>
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">Alíquota PIS atual (decimal)</label>
            <input type="number" step="0.0001" name="aliq_pis_atual" value={form.aliq_pis_atual} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">Alíquota COFINS atual (decimal)</label>
            <input type="number" step="0.0001" name="aliq_cofins_atual" value={form.aliq_cofins_atual} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">UF Origem</label>
            <input name="uf_origem" value={form.uf_origem} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">UF Destino</label>
            <input name="uf_destino" value={form.uf_destino} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">NCM</label>
            <input name="ncm" value={form.ncm ?? ""} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">CFOP</label>
            <input name="cfop" value={form.cfop ?? ""} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm text-slate-600 mb-1">Movimento</label>
            <input name="movimento" value={form.movimento ?? ""} onChange={handleChange}
              className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>

          <div className="md:col-span-3">
            <label className="block text-sm text-slate-600 mb-1">Produto</label>
      ...
      {result && (
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-6">
          <h2 className="font-semibold text-slate-700">Resultado da simulação — Ano {result.ano}</h2>
          ...
          <div>
            <h3 className="font-semibold text-slate-700 mb-2">Por tributo</h3>
